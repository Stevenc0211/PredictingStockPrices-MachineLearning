...
def partition(X,T,rate,classification=False,shuffle=False):
    ...
    if classification is False:
        ...
        xNumber = round(value * rate)
        Xtrain = X[rowIndices[:xNumber],:]
        Ttrain = T[rowIndices[:xNumber],:]
        Xtest = X[rowIndices[xNumber:],:]
        Ttest = T[rowIndices[xNumber:],:]
        ...
        Xtest = []
        Ttest = []
        for n in np.unique(T):
            temp_X = X[np.where(T == n)[0]]
            temp_T = T[np.where(T == n)[0]]
        
            value = temp_X.shape[0]
            rowIndices = np.arange(temp_X.shape[0])
            if shuffle == True:
                np.random.shuffle(rowIndices)
            xNumber = round(value * rate)
            X_trainTemp = temp_X[rowIndices[:xNumber],:]
            T_trainTemp = temp_T[rowIndices[:xNumber],:]
            X_test = temp_X[rowIndices[xNumber:],:]
            T_test = temp_T[rowIndices[xNumber:],:]
            
            Xtrain.append(X_trainTemp)
            Ttrain.append(T_trainTemp)
            Xtest.append(X_test)
            Ttest.append(T_test)

        return np.vstack(Xtrain), np.vstack(Ttrain), np.vstack(Xtest), np.vstack(Ttest)
